...
class Workspace(object):
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if self.step > self.cfg.num_seed_steps and self.step % self.cfg.save_frequency == 0:
                self.agent.save_model(output=self.work_dir, step=self.step)
            #print(done, action, obs, reward)
    
    def collect_sample(self, checkpoint_step, num_episodes, action_noise=0.0):
        if checkpoint_step > 0:
            self.agent.load_model(output=self.work_dir, step=checkpoint_step)
        num_samples = 0
        for episode in range(num_episodes):
            ...
            while not done:
                with utils.eval_mode(self.agent):
                    action = self.agent.act(obs, sample=True)
                    noise = np.random.normal(0, self.env.max_action * action_noise, size=action.shape)
                    action = (action + noise).clip(-self.env.max_action, self.env.max_action)
                new_obs, reward, done, _ = self.env.step(action)
                trj.append([obs, action, reward, new_obs])
                obs = new_obs
                episode_reward += reward
