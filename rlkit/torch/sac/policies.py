...
class ContextPolicyWrapper(Wrapper, Policy):
    def __init__(self, policy, latent_dim=None):
    ...
    def get_action(self, observation):
        if self.z is None:
            self.z = ptu.zeros(self.latent_dim)
        z = self.z
        in_ = torch.cat([observation, z.unsqueeze(0)], dim=1)
