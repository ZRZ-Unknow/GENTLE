...
def log(s, with_prefix=True, with_timestamp=True, tb_writer=None):
    out = s
    if with_prefix:
        out = _prefix_str + out
    if with_timestamp:
...
def save_itr_data(itr, **kwargs):
    file_name = osp.join(_snapshot_dir+'/saved_zs', 'data_epoch_%d.csv' % itr)
    os.makedirs(_snapshot_dir+'/saved_zs', exist_ok=True)
...
def save_contexts(itr, contexts, filename):
    filename = osp.join(_snapshot_dir+'/saved_zs', filename)
    os.makedirs(_snapshot_dir+'/saved_zs', exist_ok=True)
    np.save(filename, contexts)
