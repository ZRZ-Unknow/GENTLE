...
from rlkit.samplers.util import rollout, offline_sample, offline_rollout, offline_rollout_with_context
...
class OfflineInPlacePathSampler(InPlacePathSampler):
    ...
    def obtain_samples_with_context(self, context_buffer, deterministic=False, max_samples=np.inf, max_trajs=np.inf, accum_context=True, resample=1,
                       rollout=False):
        """
        Obtains samples from saved trajectories until either we reach either max_samples transitions or
        num_traj trajectories.
        ...
        n_trajs = 0
        while n_steps_total < max_samples and n_trajs < max_trajs:
            #oracle_env.reset_task(self.env._goal_idx)
            path = offline_rollout_with_context(self.env, policy, context_buffer, max_path_length=self.max_path_length, accum_context=accum_context)
            # save the latent context that generated this trajectory
            path['context'] = policy.z.detach().cpu().numpy()
            paths.append(path)
            n_steps_total += len(path['observations'])
