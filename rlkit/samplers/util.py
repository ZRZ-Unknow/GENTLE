...
def offline_rollout_with_context(env, agent, context_buffer, max_path_length=np.inf, accum_context=True, animated=False, save_frames=False):
    # perform online rollout as in rollout()
    # If accum_context=True, aggregate offline context
    batch_dict = context_buffer.random_batch(max_path_length)
    observations = []
    actions = []
    rewards = []
    terminals = []
    ...
    while path_length < max_path_length:
        with torch.no_grad():
            a, agent_info = agent.get_action(o)
        next_o, r, d, env_info = env.step(a)
        #_, oracle_r, _, _ = oracle_env.oracle_step(o, a)
        #diff = abs(r-oracle_r)
        #assert diff > 1e-4, f"step{path_length}, r {r}, oracle_r {oracle_r}, diff{abs(r - oracle_r)}")
        observations.append(o)
        rewards.append(r)
        terminals.append(d)
        actions.append(a)
