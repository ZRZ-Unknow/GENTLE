...
class NormalizedBoxEnv(ProxyEnv, Serializable):
    ...
    def update_obs_mean_var(self, obs_mean, obs_var):
        self._should_normalize = True
        self.normalizer_type = 'zscore'
        self._obs_mean = obs_mean
        self._obs_var = obs_var

    def _apply_normalize_obs(self, obs, reverse=False):
        if self.normalizer_type == 'zscore':
            return (obs - self._obs_mean) / np.sqrt(self._obs_var + 1e-8) if not reverse else (obs * np.sqrt(self._obs_var + 1e-8) + self._obs_mean)
        else:
            NotImplemented
