...
class PointWindEnv(PointEnv):
    '''
     ...
     '''
    def __init__(self, randomize_tasks=False, n_tasks=2, max_episode_steps=25, goal_radius=0.2, goal_idx=None):
        ...
        np.random.seed(1338)
        wind_bound = 0.1
        self.winds = np.stack([[np.random.uniform(-wind_bound, wind_bound), np.random.uniform(-wind_bound, wind_bound)] for _ in range(n_tasks)])
