...
for file in os.listdir(os.path.dirname(__file__)):
    ...
    if mjpro is not None:
        if file.endswith('.py') and not file.startswith('_') and not file.startswith('non_mujoco'):
            if mjpro != 'mjpro131':
                if not file.startswith('walker_rand_params') and not file.startswith('hopper_rand_params'):
                    module = file[:file.find('.py')]
                    importlib.import_module('rlkit.envs.' + module)
                    _tabulate(module)
            else:
                if file.startswith('walker_rand_params') or file.startswith('hopper_rand_params'):
                    module = file[:file.find('.py')]
                    importlib.import_module('rlkit.envs.' + module)
                    _tabulate(module)
    else:
