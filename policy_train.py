...
def main(config, gpu, docker, debug, eval, goal_idx=0, seed=0):
    ...
    if variant['env_params']['n_tasks'] > 1:
        ...
        for i in range(0, n_tasks, n):
            tmp = list(range(i, i+n if i+n <= n_tasks else n_tasks))
            #experiment(variant=variant, cfg=cfg, goal_idx=i)
            p.starmap(experiment, product([variant], [cfg], tmp))
    else:
